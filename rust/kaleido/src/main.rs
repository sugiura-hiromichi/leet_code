...
impl Buf {
    ...
    fn gettok(&mut self) -> Token {
        ...
        if self.lst_chr != None
            && (self.lst_chr.unwrap().is_numeric() || self.lst_chr.unwrap() == '.')
        {
...
fn parse_identifier_exp(buf: &mut Buf) -> Result<ExprAST, ()> {
    ...
    if buf.curtok.clone().unwrap() != Token::Other('(') {
        ...
        while buf.curtok.clone().unwrap() != Token::Other(')') {
            ...
            if buf.curtok.clone().unwrap() != Token::Other(',') {
                return log_err(
                    "in parse_identifier_exp: Expected ')' or ',' in argument list".to_string(),
                );
            };
            buf.peek_token();
        }
...
fn parse_bin_op_rhs(buf: &mut Buf, mut lhs: ExprAST, cur_op: Op) -> Result<ExprAST, ()> {
    loop {
        ...
        // if last_op binds less tightly with rhs than the operator after rhs, let the pending
        // oerator take rhs as its lhs because current rhs have to be a part of actual one
        ...
        if last_op < next_op {
            let lsp_r = match last_op {
                ...
                _ => {
                    return log_err("in parse_bin_op_rhs: parsing should be uncertain".to_string())
                }
            };
